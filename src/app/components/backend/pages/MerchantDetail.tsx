  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200">
      <div className="px-5 py-3 border-b border-gray-200"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><Package className="w-3.5 h-3.5 text-gray-400" /><h3 className="text-sm font-semibold text-gray-700">Bundles & Referrals</h3></div>
        <div className="relative"><button onClick={() => setIssueDropdown(!issueDropdown)} className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-gray-600 hover:text-[#4318FF] hover:bg-indigo-50 border border-gray-200 rounded-[6px] transition-colors"><Gift className="w-3.5 h-3.5" />Issue Credit<ChevronDown className="w-3 h-3 ml-0.5" /></button>
  ...
  return (
    <div className="border border-gray-200 rounded-lg p-3 flex items-center justify-between">
      ...
      <div className="flex items-center gap-2 ml-3 shrink-0">
        <button onClick={() => onPreview(tplId)} className="text-xs text-[#4318FF] hover:underline">Preview</button>
        <button className="px-3 py-1.5 bg-[#4318FF] text-white text-xs font-medium rounded-[6px] hover:bg-indigo-700 transition-colors flex items-center gap-1"><Send className="w-3 h-3" />Send</button>
      </div>
    </div>
  );
}
        ...
        {activeTab === 'overview' && (
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              <div className="bg-white rounded-lg border border-gray-200">
                <div className="px-5 py-4 border-b border-gray-200"><div className="flex items-center gap-2"><StickyNote className="w-4 h-4 text-[#4318FF]" /><h2 className="text-lg font-semibold text-gray-900">Notes & Tasks</h2></div></div>
                <div className="px-5 py-4 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">{tasks.map(t => (
                      <div key={t.id} className="flex items-start gap-2.5 group">
                        <button onClick={() => setTasks(p => p.map(x => x.id === t.id ? { ...x, done: !x.done } : x))} className={`mt-0.5 w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors ${t.done ? 'bg-[#4318FF] border-[#4318FF]' : 'border-gray-300 hover:border-[#4318FF]'}`}>{t.done && <CheckCircle className="w-3 h-3 text-white" />}</button>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <input type="text" value={newTask} onChange={e => setNewTask(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && newTask.trim()) { setTasks(p => [...p, { id: `t-${Date.now()}`, text: newTask.trim(), due: 'Set date...', done: false }]); setNewTask(''); } }} placeholder="Add a task..." className="flex-1 text-sm border border-gray-200 rounded-[6px] px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-[#4318FF]/20 focus:border-[#4318FF] placeholder:text-gray-400" />
                      <button onClick={() => { if (newTask.trim()) { setTasks(p => [...p, { id: `t-${Date.now()}`, text: newTask.trim(), due: 'Set date...', done: false }]); setNewTask(''); } }} className="px-3 py-1.5 bg-[#4318FF] text-white text-xs font-medium rounded-[6px] hover:bg-indigo-700 transition-colors">Add</button>
                    </div>
                  </div>
                  <div className="border-t border-gray-100 pt-4">
                    ...
                    <div className="flex items-start gap-2 mt-2">
                      <textarea value={newNote} onChange={e => setNewNote(e.target.value)} placeholder="Add a note..." rows={2} className="flex-1 text-sm border border-gray-200 rounded-[6px] px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-[#4318FF]/20 focus:border-[#4318FF] placeholder:text-gray-400 resize-none" />
                      <button onClick={() => { if (newNote.trim()) { setNotes(p => [{ id: `n-${Date.now()}`, text: newNote.trim(), author: 'You', date: 'Just now' }, ...p]); setNewNote(''); } }} className="px-3 py-1.5 bg-[#4318FF] text-white text-xs font-medium rounded-[6px] hover:bg-indigo-700 transition-colors">Add</button>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {/* Payments Snapshot */}
              <div className="bg-white rounded-lg border border-gray-200">
                <div className="px-5 py-3 border-b border-gray-200 flex items-center justify-between"><div className="flex items-center gap-2"><CreditCard className="w-4 h-4 text-[#4318FF]" /><h3 className="text-sm font-semibold text-gray-700">Payments Snapshot</h3></div><button onClick={() => navigate('/residuals')} className="text-xs text-[#4318FF] hover:underline underline-offset-2 font-medium flex items-center gap-1">Full Residuals <ExternalLink className="w-3 h-3" /></button></div>
              ...
              {/* Capital Snapshot */}
              <div className="bg-white rounded-lg border border-gray-200">
                <div className="px-5 py-3 border-b border-gray-200 flex items-center justify-between"><div className="flex items-center gap-2"><Banknote className="w-4 h-4 text-[#4318FF]" /><h3 className="text-sm font-semibold text-gray-700">Capital Snapshot</h3></div><button onClick={() => navigate('/capital')} className="text-xs text-[#4318FF] hover:underline underline-offset-2 font-medium flex items-center gap-1">Full Capital <ExternalLink className="w-3 h-3" /></button></div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-2">
              {([{ id: 'payments' as const, label: 'Payments', icon: <CreditCard className="w-3.5 h-3.5" /> }, { id: 'capital' as const, label: 'Capital', icon: <Banknote className="w-3.5 h-3.5" /> }, { id: 'website' as const, label: 'Website', icon: <Globe className="w-3.5 h-3.5" /> }, { id: 'lens' as const, label: 'Lens AI', icon: <Brain className="w-3.5 h-3.5" /> }]).map(p => (
                <button key={p.id} onClick={() => setOutreachProduct(p.id)} className={`flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-[6px] border transition-all ${outreachProduct === p.id ? 'bg-[#4318FF] text-white border-[#4318FF]' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}>{p.icon}{p.label}</button>
              ))}
            </div>
            ...
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg border border-indigo-200 px-5 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Megaphone className="w-5 h-5 text-[#4318FF]" />
                <div>
              ...
              <button onClick={() => navigate('/outreach')} className="px-4 py-2 bg-[#4318FF] text-white text-sm font-medium rounded-[6px] hover:bg-[#3311DD] transition-colors flex items-center gap-1.5 shrink-0"><Send className="w-3.5 h-3.5" />Go to Outreach</button>
            </div>
              ...
              <>
                <div className="bg-white rounded-lg border border-gray-200">
                  <div className="px-5 py-16 flex flex-col items-center justify-center text-center">
                    ...
                    <button className="px-5 py-2.5 bg-[#0284C7] text-white text-sm font-medium rounded-[6px] hover:bg-[#0369A1] transition-colors">Send Website Offer</button>
                  </div>
                </div>
                <div className="bg-gray-50 rounded-lg border border-gray-200 px-5 py-4 flex items-center justify-between">
                  ...
                  <button onClick={() => { setActiveTab('outreach'); setOutreachProduct('website'); }} className="text-xs text-[#4318FF] hover:underline font-medium flex items-center gap-1">Go to Outreach <ExternalLink className="w-3 h-3" /></button>
                </div>
              </>
            ) : (
              <>
                ...
                <div className="bg-white rounded-lg border border-gray-200">
                  ...
                  <div className="px-5 py-4">
                    ...
                    <a href="https://www.sunrisecafe.com" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-sm font-medium text-[#0284C7] hover:underline underline-offset-2 mb-4">www.sunrisecafe.com <ExternalLink className="w-3.5 h-3.5" /></a>
              ...
              <>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                  <div className="lg:col-span-3 space-y-6">
                    ...
                    <div className="bg-white rounded-lg border border-gray-200">
                      <div className="px-5 py-4 border-b border-gray-200 flex items-center justify-between"><h2 className="text-sm font-semibold text-gray-900">Recent API Calls</h2><button className="text-xs text-[#4318FF] font-medium hover:underline">View All</button></div>
