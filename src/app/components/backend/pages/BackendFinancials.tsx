...
const revenueBreakdown = [
  { label: 'MCA Interest Income', value: 285000, pct: 53.2, color: 'bg-brand-500' },
    ...
    <div className="px-6 py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <span className="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium bg-red-50 text-red-700 border border-red-200 rounded-[8px]">
            Super Admin
          </span>
        </div>
      ...
      <div className="bg-white rounded-[8px] border border-gray-200">
        <div className="px-5 py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          ...
          {alertWeeks.length > 0 && (
            <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 border border-amber-200 rounded-[8px] text-xs text-amber-700">
            ...
            <table className="w-full text-sm">
                ...
                {fundingSources.map((src) => {
                  ...
                  return (
                    <tr key={src.name} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right font-medium text-brand">{src.returnPct.toFixed(1)}%</td>
                    </tr>
                  );
                })}
              </tbody>
              <tfoot>
                <tr className="bg-gray-50 border-t-2 border-gray-200">
                  ...
                  <td className="px-4 py-3 text-right font-semibold text-brand">{(fundingSources.reduce((s, f) => s + f.returnPct, 0) / fundingSources.length).toFixed(1)}%</td>
                </tr>
              </tfoot>
            </table>
          </div>
      ...
      <div className="bg-white rounded-[8px] border border-gray-200">
            ...
            <tbody className="divide-y divide-gray-100">
              {transactions.map((tx, i) => (
                <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex px-2 py-1 text-xs border rounded-[8px] ${
                        tx.type === 'Income'
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 text-sm text-gray-500">
          ...
          <button className="text-brand hover:text-brand-hover text-sm font-medium">View All Transactions</button>
        </div>
      </div>
    </div>
  );
